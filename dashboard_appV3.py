...
# Maps regime_engine.regime_color() output ("green"/"red"/"orange"/"blue"/"gray")
# onto the QuantLab design-token palette so Regime Intelligence stays visually
...
def get_price_data(tickers: tuple[str, ...]) -> pd.DataFrame:
    ...
    return simulate_price_history(list(tickers))
...
def render_sidebar() -> None:
    ...
    with st.sidebar:
        ...
        st.markdown(
            f'<div class="ql-section-header" style="margin-top:16px;">Portfolio</div>',
            unsafe_allow_html=True,
        )
        ...
        for icon, page in portfolio_pages:
            if st.button(
                f"{icon} {page}",
                key=f"nav_{page}",
...
def render_regime_intelligence() -> None:
    ...
    Classifies the prevailing market regime for the active portfolio
    universe via `regime_engine.detect_market_regime` and surfaces the
    full `RegimeResult` payload: the headline classification and
    confidence score, the canonical statistics that drove the call
    (annualised return / volatility, max drawdown, 63-day and 252-day
    momentum), the supporting diagnostics and evidence, a narrative
    explanation, and actionable portfolio guidance.
    """
    page_header(
        "Regime Intelligence",
        "Detect the current market regime and translate it into portfolio guidance.",
    )
    ...
    with st.spinner("Detecting market regime …"):
        try:
            ...
            result: RegimeResult = detect_market_regime(prices)
        except Exception as exc:
            st.error(f"Regime detection failed: {exc}")
            return

    regime_hex = REGIME_COLOR_MAP.get(regime_color(result.regime), COBALT2)
    confidence_band = (
        "High" if result.confidence >= 0.66
        else "Moderate" if result.confidence >= 0.33
        else "Low"
    )

    # ── Headline regime + confidence ────────────────────────────────────────
    section_header("Current Regime")
    col_regime, col_conf = st.columns([3, 2])
    with col_regime:
        st.markdown(
            f"""
            <div class="ql-card" style="border-color:{regime_hex}55; min-height:140px;">
                <div class="ql-card-label">Current Regime</div>
                <div class="ql-card-value" style="color:{regime_hex};">{result.regime.value}</div>
                <div class="ql-card-sub">{regime_description(result.regime)}</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
    with col_conf:
        metric_card(
            ...
            f"{confidence_band} confidence in this classification",
            "neutral",
        )
    ...
    with c1:
        metric_card(
            "Annual Return", f"{result.annual_return:.2%}", "Geometric, annualised",
            "positive" if result.annual_return >= 0 else "negative",
        )
    with c2:
        metric_card(
            "Annual Volatility", f"{result.annual_volatility:.2%}", "Daily σ × √252",
            "neutral",
        )
    with c3:
        metric_card(
            "Drawdown", f"{result.drawdown:.2%}", "Max peak-to-trough",
            "negative",
        )
    with c4:
        metric_card(
            "63-Day Momentum", f"{result.momentum_63d:.2%}", "~3 month trailing return",
            "positive" if result.momentum_63d >= 0 else "negative",
        )
    with c5:
        metric_card(
            "252-Day Momentum", f"{result.momentum_252d:.2%}", "~1 year trailing return",
            "positive" if result.momentum_252d >= 0 else "negative",
        )

    # ── Explanation ───────────────────────────────────────────────────────────
    section_header("Explanation")
    explanation = getattr(result, "explanation", "") or (
        f"The portfolio is classified as {result.regime.value} with "
        f"{result.confidence:.0%} confidence based on annualised return, "
        f"volatility, drawdown, and dual-horizon momentum statistics."
    )
    alert(explanation, "info")
    ...
    if evidence:
        ...
        st.caption("No supporting evidence statements were returned for this result.")

    # ── Portfolio guidance ────────────────────────────────────────────────────
    section_header("Portfolio Guidance")
    portfolio_implications = getattr(result, "portfolio_implications", "") or (
        "No regime-specific portfolio guidance was returned for this result."
    )
    alert(portfolio_implications, "success")
    ...
    if diagnostics:
        diagnostics_df = pd.DataFrame(
            {
                "Metric": list(diagnostics.keys()),
                "Value": list(diagnostics.values()),
            }
        )
        st.dataframe(diagnostics_df, hide_index=True, use_container_width=True)
    else:
        st.caption("No diagnostics payload was returned for this result.")

    logger.info(
        "Regime Intelligence rendered | regime=%s | confidence=%.2f",
